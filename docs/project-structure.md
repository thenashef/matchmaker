...
- **`server/jms/`** — the async server→client push layer, following the same interface + real-impl (+ test-fake where useful) pattern: `GameEventPublisher` (interface, `publishToPlayer(int userId, GameEventDTO event)`) / `ActiveMqGameEventPublisher` — publishes a `GameEventDTO` as a JMS `ObjectMessage` to a per-player `Queue` named `player.{userId}.events`; `JmsConnectionFactory` builds a connection against an embedded, in-process ActiveMQ broker (`vm://matchmaker-<uuid>`, unique per call) — no Docker, no separate broker process required, ever. `JmsPublishException` (unchecked) wraps a `JMSException` from a failed publish; callers (currently only `PlayerServiceImpl.joinQueue()`) catch it and log rather than fail the caller's own already-successful result. Test-only fakes live in `src/test/java/.../jms/`: `InMemoryGameEventPublisher` (records what was published, for assertions) and `FailingGameEventPublisher` (always throws, for proving a publish failure doesn't break the caller). **Only `MATCH_FOUND` exists as an event today** (`GameEventType`, `common/enums/`) — there's no per-session JMS *Topic* yet (the spec's design for "opponent made a move"/chat/resign/game-end broadcasts), only this per-player *Queue* used solely for the pre-session `MATCH_FOUND` case. That topic is explicitly deferred — see "Next Steps" in `build-plan.md`.
